...
def test_data_dir(tmp_path):
    ...
    df1 = pd.DataFrame({
        'Job Title': ['Customer Success Manager', 'Customer Experience Lead', 'Technical Customer Success Manager'],
        'Industry': ['SaaS', 'Technology', 'Technology'],
        'Responsibilities': [
        ...
        'Salary Range': ['$80,000 - $120,000', '$90,000 - $130,000', '$100,000 - $150,000'],
        'Job Type': ['Full-time', 'Full-time', 'Full-time']
    })
    
    df2 = pd.DataFrame({
        'Job Title': ['Implementation Specialist', 'Customer Support Lead', 'Customer Success Operations'],
        'Industry': ['SaaS', 'Technology', 'Technology'],
        'Responsibilities': [
        ...
        'Salary Range': ['$75,000 - $110,000', '$85,000 - $125,000', '$90,000 - $130,000'],
        'Job Type': ['Full-time', 'Full-time', 'Full-time']
    })
...
def test_initialize_data(data_manager):
    """Test data initialization for CS/CX roles"""
    # We have 6 CS/CX focused jobs in the test data
    assert len(data_manager.job_requirements) == 6
    
    # Check specific job titles from our CS/CX test data
    assert 'Customer Success Manager' in data_manager.job_requirements
    assert 'Customer Experience Lead' in data_manager.job_requirements
    assert 'Technical Customer Success Manager' in data_manager.job_requirements
    assert 'Implementation Specialist' in data_manager.job_requirements
    assert 'Customer Support Lead' in data_manager.job_requirements
    assert 'Customer Success Operations' in data_manager.job_requirements
    
    # Verify CS/CX job details
    csm_job = data_manager.job_requirements['Customer Success Manager']
    assert 'customer success' in csm_job.required_skills
    assert 'crm' in csm_job.required_skills
    assert 'communication' in csm_job.required_skills

def test_data_integrity(data_manager):
    """Verify CS/CX test data integrity"""
    # Check all required CS/CX jobs are present
    required_jobs = ['Customer Success Manager', 'Customer Experience Lead', 
